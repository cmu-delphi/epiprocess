...
new_grouped_epi_archive <- function(ungrouped, vars, drop) {
  ...
  assert_class(ungrouped, "epi_archive2")
  ...
  return(structure(
    ...
    class = c("grouped_epi_archive2", "epi_archive2")
  ))
}

#' @export
print.grouped_epi_archive2 <- function(grouped_epi_archive, class = TRUE) {
...
#' @export
group_by.grouped_epi_archive2 <- function(
    grouped_epi_archive,
    ...,
    .add = FALSE,
  ...
  if (!.add) {
    ...
    # `?dplyr::group_by`)
    detailed_mutate <- epix_detailed_restricted_mutate2(grouped_epi_archive$private$ungrouped, ...)
...
#'
#' @export
group_by_drop_default.grouped_epi_archive2 <- function(grouped_epi_archive) {
...
#' @export
groups.grouped_epi_archive2 <- function(grouped_epi_archive) {
...
#' @export
ungroup.grouped_epi_archive2 <- function(grouped_epi_archive, ...) {
  if (rlang::dots_n(...) == 0L) {
...
#' @export
truncate_versions_after.grouped_epi_archive2 <- function(grouped_epi_archive, max_version) {
...
#' Truncate versions after a given version, grouped
#' @export
epix_truncate_versions_after.grouped_epi_archive2 <- function(grouped_epi_archive, max_version) {
  cloned_group_epi_archive <- clone(grouped_epi_archive, deep = TRUE)
  return((truncate_versions_after(cloned_group_epi_archive, max_version)))
...
#'  env missing_arg
#' @export
slide.grouped_epi_archive2 <- function(grouped_epi_archive, f, ..., before, ref_time_values,
  ...
  x <- lapply(ref_time_values, function(ref_time_value) {
    ...
    as_of_raw <- as_of(grouped_epi_archive$private$ungrouped,
      ref_time_value,
    ...
    if (!all_versions) {
      ...
      group_modify_fn <- function(.data_group, .group_key,
        ...
        .data_group_archive <- clone(as_of_archive)
...
#'
#' @export
is_grouped_epi_archive2 <- function(x) {
  inherits(x, "grouped_epi_archive2")
}


#' @export
clone.grouped_epi_archive2 <- function(x, deep = FALSE) {
  # TODO: Finish.
  if (deep) {
    ungrouped <- clone(x$private$ungrouped, deep = TRUE)
  } else {
    ungrouped <- x$private$ungrouped
  }
