...
apply_result_common_sizer <- function(quosure_result_raw, sizer, results_env) {
  ...
  return(list(quosure_result_recycled, common_size))
  # return(quosure_result_recycled) # TODO + mutable sizer object?
}

# new_common_size_result_builder <- function() {
#   results_env <- new.env(parent = emptyenv())
#   common_size <- NULL
#   function(quosure_result_raw) {
#   }
#   but what about binding?
# }

new_result_common_sizer <- function(results_env) {
  common_size <- NULL
  function(quosure_result_raw) {
    # We want something like `dplyr_col_modify()` but allowing recycling
    # of previous computations and updating `results_env` and unpacking
    ...
    return(quosure_result_recycled)
  }
}

apply_comp_quosures <- function(data_mask, comp_quos, common_size) {
  # common_size <- NULL
  ...
  results_env <- new.env(parent = emptyenv())
  sizer <- new_result_common_sizer(results_env)
  ...
  for (quosure_i in seq_along(comp_quos)) {
    ...
    if (is.null(quosure_result_raw)) {
      ...
      #   } # else `common_size` remains NULL
      # }
      # temp <- apply_single_size_spec(quosure_result_raw, common_size, results_env)
      # quosure_result_recycled <- temp[[1L]]
      # common_size <- temp[[2L]]
      # quosure_result_recycled <- apply_single_size_spec(quosure_result_raw, common_size, results_env)
      quosure_result_recycled <- sizer(quosure_result_raw)
