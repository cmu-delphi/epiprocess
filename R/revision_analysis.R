...
#' @importFrom dplyr mutate group_by arrange filter if_any all_of across
#' @importFrom dplyr pull pick c_across everything ungroup summarize if_else %>%
revision_analysis <- function(epi_arch,
  ...
  if (!return_only_tibble) {
    revision_behavior <- structure(list(
      ...
      range_time_values = archive_col_range(epi_arch, "time_value"),
      signal_variable = arg,
      drop_nas = drop_nas,
      time_type = time_type,
      total_na = total_na,
