...
#' toy_epi_archive
#' @name epi_archive
# TODO: Figure out where to actually put this documentation
NULL
...
new_epi_archive2 <- function(
  ...
  structure(
    list(
      ...
      private = list() # TODO: to be encapsulated with guard-rails later
    ),
    class = "epi_archive2"
  )
}
...
#' @export
print.epi_archive2 <- function(epi_archive, class = TRUE, methods = TRUE) {
  cli_inform(
    c(
...
#' @export
as_of.epi_archive2 <- function(epi_archive, max_version, min_time_value = -Inf, all_versions = FALSE) {
  other_keys <- setdiff(
    key(epi_archive$DT),
...
fill_through_version.epi_archive2 <- function(
    epi_archive,
    fill_versions_end,
    how = c("na", "locf")) {
...
truncate_versions_after.epi_archive2 <- function(
    epi_archive,
    max_version) {
  if (!test_set_equal(class(max_version), class(epi_archive$DT$version))) {
...
group_by.epi_archive2 <- function(epi_archive, ..., .add = FALSE, .drop = dplyr::group_by_drop_default(epi_archive)) {
  # `add` makes no difference; this is an ungrouped `epi_archive`.
  detailed_mutate <- epix_detailed_restricted_mutate2(epi_archive, ...)
  assert_logical(.drop)
  if (!.drop) {
...
#'   Default is `FALSE`.
slide.epi_archive2 <- function(epi_archive, f, ..., before, ref_time_values,
...
#'   other_keys = "county"
#' )
as_epi_archive2 <- function(x, geo_type, time_type, other_keys,
...
is_epi_archive2 <- function(x, grouped_okay = FALSE) {
  inherits(x, "epi_archive2") || grouped_okay && inherits(x, "grouped_epi_archive2")
}
...
#' @export
clone.epi_archive2 <- function(epi_archive, deep = FALSE) {
  # TODO: Finish.
  if (deep) {
