...
epi_slide_opt_edf_one_epikey <- function(inp_tbl,
                                         ...
                                         before_n_steps, after_n_steps, unit_step, time_type,
                                         out_colnames,
  ...
  if (before_n_steps == Inf) {
    ...
    slide_t_min <- out_t_min - before_n_steps * unit_step
    slide_start_padding_n <- before_n_steps # perf: avoid time_minus_time_in_n_steps
  }
  ...
  if (f_from_package == "data.table") {
    if (before_n_steps == Inf) {
      ...
      out_cols <- f_dots_baked(slide[, in_colnames], before_n_steps + after_n_steps + 1L)
      if (after_n_steps != 0L) {
    ...
    for (col_i in seq_along(in_colnames)) {
      slide[[out_colnames[[col_i]]]] <- f_dots_baked(slide[[in_colnames[[col_i]]]], before = before_n_steps, after = after_n_steps)
    }
  } else {
    cli_abort(
...
epi_slide_opt.epi_df <- function(.x, .col_names, .f, ...,
  ...
  result <- .x %>%
    group_modify(function(grp_data, grp_key) {
      epi_slide_opt_edf_one_epikey(grp_data, names_info$input_col_names, f_dots_baked, f_from_package, before_n_steps, after_n_steps, unit_step, time_type, names_info$output_col_names, NULL, ref_time_values)
    }) %>%
