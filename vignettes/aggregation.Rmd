---
title: 4. Aggregate signals over space and time
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{4. Aggregate signals over space and time}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

Aggregation, both time-wise and geo-wise, are common tasks when working with
epidemiological data sets. To demonstrate how to carry out these kinds of tasks
with `epi_df` objects, we'll work with county-level reported COVID-19 cases in
MA and VT.
...
# Fetch only counties from VT and NH, then append names columns as well
x <- covidcast_signal(data_source = "jhu-csse", 
                      signal = "confirmed_incidence_num",
                      start_day = "2020-06-01", 
                      end_day = "2021-12-31",
                      geo_type = "county", 
                      geo_values = y$geo_value) %>% 
...
bunch of useful tools. A `tsibble` is basically a tibble (data frame) but with
two specially-marked columns: an *index* column representing the time variable
(defining an order from past to present), and a *key* column identifying a
unique observational unit for each time point. In fact, the key in a `tsibble`
can be made up of any number of columns, not just a single one.
...
As we can see, there are duplicate county names between MA and VT, which caused
the error.
...
xt <- as_tsibble(x) %>% filter(cases >= 5) 
```
...
(Other approaches, such as LOCF: last-observation-carried-forward, could be
accomplished by first filling with `NA` values and then following up with a
...
Note that the time series for Addison, VT only starts on October 5, 2020, even
...
time scales using functions like `index_by()` from `tsibble` and then aggregate
using the usual `dplyr` verbs like `summarize()`. The `index_by()` function
modifies the index variable in the given object by applying a suitable time
transformation to extract a coarser time scale (say, moving from days to weeks,
or weeks to months, and so on).

*todo: we should implement a `yearepiweek()` function in `epiprocess`, modeled
after `yearweek()` from `tsibble`, but for epiweeks. once done, describe that
here and replace `yearweek()` in the example below*
