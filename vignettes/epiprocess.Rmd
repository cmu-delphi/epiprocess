...
[`epi_df`](reference/epi_df.html) gives more details about this data format. 
...
example, we'll work with daily cumulative COVID-19 cases for 4 states (CA, FL,
NY, and TX) in the U.S., over time span from mid 2020 to early 2022, and we'll
use the [`covidcast`](https://cmu-delphi.github.io/covidcast/covidcastR/)
...
x <- as_epi_df(cases, 
  ...
  select(geo_value, time_value, cases_cum = value)
...
as of the time specified `as_of`. For example, if `as_of` is January 31, 2022,
...
As last example, we'll look at a few more data sets just to show how we might
get them into `epi_df` format. Data on daily new (not cumulative) SARS cases 
