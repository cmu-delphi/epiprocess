...
dt <- tibble::as_tibble(archive_cases_dv_subset)

locf_omitted <- as_epi_archive(dt)
locf_included <- as_epi_archive(dt, compactify = FALSE)

head(tibble::as_tibble(locf_omitted))
head(tibble::as_tibble(locf_included))
```
...
```{r}
nrow(tibble::as_tibble(locf_included_2))
nrow(tibble::as_tibble(locf_omitted_2))
```
...
iterate_filter <- function(my_ea) {
  archive_rows <- tibble::as_tibble(my_ea)
  for (i in 1:1000) {
    filter(archive_rows, version >= as.Date("2020-01-01") + i)
  }
}
...
iterate_as_of <- function(my_ea) {
  min_time_value <- min(tibble::as_tibble(my_ea)$time_value)
  for (i in 1:1000) {
    my_ea %>% epix_as_of(min_time_value + i - 1000)
  }
}
