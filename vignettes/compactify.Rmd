...
LOCF can mar the performance of dataset operationsm as evidenced with the
system time.

```{r}
iterate_test <- function(my_dt) {
  for (i in 1:200) {
    filter(dt,version >= as.Date("2020-01-01") + i)
  }
}

system.time(iterate_test(locf_included$DT))

system.time(iterate_test(locf_omitted$DT))
```
