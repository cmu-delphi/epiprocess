...
The `slided_cor()` function operates on an `epi_signal` object, and it requires
further specification of the variables to correlate, in its next two arguments 
(`var1` and `var2`). In general, there are two ways to calculate correlations. 
The first way is to "slice by time": this calculates, for each time value, the
...
z1 <- sliced_cor(x, case_rates, death_rates, by = time_value)
...
*future*. Using the `dt1` parameter in `sliced_cor()`, we can lag case rates
...
```{r, message = FALSE, warning = FALSE}
z2 <- sliced_cor(x, case_rates, death_rates, by = time_value, dt1 = -10)
...
```{r, message = FALSE, warning = FALSE}
z1 <- sliced_cor(x, case_rates, death_rates, by = geo_value)
z2 <- sliced_cor(x, case_rates, death_rates, by = geo_value, dt1 = -10)
...
z <- map_dfr(lags, function(lag) {
  sliced_cor(x, case_rates, death_rates, by = geo_value, dt1 = -lag) %>% 
    mutate(lag = lag) 
  })
