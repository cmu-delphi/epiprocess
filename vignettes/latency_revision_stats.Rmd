...
latency_stats = function(window_data, group_key, ref_time_value) {
  window_data %>%
    ...
    summarize(
      ...
      # FIXME this still looks buggy; why don't we have the same all-geo event lines as in the above?
      na_dup_latency =
        # (don't count NAs before the first copy of the last non-na)
        sum(cumsum(is_na_same[rev(cumsum(rev(!is_na_same))) == 0L]) != 0L) - 1L + nominal_latency,
      na0dup_latency =
        # (don't count NAs and 0s before the first copy of the last non-na0)
        sum(cumsum(is_na0same[rev(cumsum(rev(!is_na0same))) == 0L]) != 0L) - 1L + nominal_latency
    )
}
