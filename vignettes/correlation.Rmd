...
correlations between two signals, over space or time, via `epi_cor()`. This
...
x <- x %>%
  ...
  epi_slide(death_rates = mean(death_rates), n = 7) %>%
  ungroup()
```
...
in a call to `epi_cor()`, via the `by` argument. This potentially leads to many
ways to compute correlations. There are always at least two ways to compute
correlations in an `epi_df`: grouping by time value, and by geo value. The
...
z1 <- epi_cor(x, case_rates, death_rates, by = time_value)
...
```{r, message = FALSE, warning = FALSE}
z2 <- epi_cor(x, case_rates, death_rates, by = time_value, dt1 = -10)
...
```{r, message = FALSE, warning = FALSE}
z1 <- epi_cor(x, case_rates, death_rates, by = geo_value)
z2 <- epi_cor(x, case_rates, death_rates, by = geo_value, dt1 = -10)
...
z <- map_dfr(lags, function(lag) {
  epi_cor(x, case_rates, death_rates, by = geo_value, dt1 = -lag) %>% 
    mutate(lag = lag) 
  })
