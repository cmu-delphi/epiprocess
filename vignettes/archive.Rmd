...
```{r}
slide_time_values = seq(as.Date("2020-08-01"), 
                        as.Date("2021-12-01"), 
                        by = "1 month")
...
z <- x$slide(fc = prob_ar(case_rates, percent_cli), n = 120, 
        slide_time_values = slide_time_values)
...
k_week_ahead <- function(x, ahead = 7, as_of = TRUE) {
  if (as_of) {
      x$slide(fc = prob_ar(case_rates, percent_cli, ahead = ahead), 
              n = 120, slide_time_values = slide_time_values) %>%
        mutate(target_date = time_value + ahead, as_of = TRUE) 
  }
  else {
    x %>% 
      group_by(geo_value) %>%
      epi_slide(fc = prob_ar(case_rates, percent_cli, ahead = ahead), 
                n = 120, slide_time_values = slide_time_values) %>%
...
fc <- bind_rows(k_week_ahead(x, ahead = 7),
...
p <- ggplot(fc, aes(x = target_date, group = time_value)) +
  ...
  facet_wrap(vars(geo_value, as_of), scales = "free_y") +
  scale_x_date(minor_breaks = "month", date_labels = "%b %y") +
  labs(x = "Date", y = "Reported COVID-19 cases")
  
gginnards::append_layers(
  p, geom_line(data = z, aes(x = time_value, y = case_rates), 
