...
latest snapshot `z` that the archive can provide).
...
dt0[, (vars2):=setnafill(.SD, type="locf", cols=vars2), .SDcols=vars2, by="id"])

DT[, percent_cli = nafill(percent_cli, type = "locf"), by = .(geo_value, )]


library(data.table)
xx = data.table(t = c(rep("a",5), rep("b",5)), u = 11:20, v1 = 1:10, v2 = 1:10)
xx[3,3] = NA # Shoud be filled be LOCF
xx[6,4] = NA # Should NOT be filled by LOCF, it's the first one in group b
xx[10,3:4] = NA # Should both be filled by LOCF
xx
fillvars <- c("v1", "v2")
#xx[, (fillvars) := setnafill(.SD, type = "locf", cols = fillvars), by = "t"]
xx[, (fillvars) := setnafill(.SD, type = "locf"), .SDcols = fillvars, by = "t"]
xx
