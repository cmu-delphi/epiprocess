...
z <- x %>% 
  group_by(geo_value) %>%
  epi_slide(forecast = prob_ar(cases_7dav), n = 120, as_list_col = TRUE) %>%
...
k_week_ahead <- function(x, ahead = 7) {
  x %>% 
    group_by(geo_value) %>%
    epi_slide(forecast = prob_ar(cases_7dav, ahead = ahead), n = 120,
              as_list_col = TRUE) %>%
...
training models and making predictions historically. However, this 
...
lead to an overly optimistic sense of accuracy; see, for example, [McDonald et
al. (2021)](https://www.pnas.org/content/118/51/e2111453118/), and references
