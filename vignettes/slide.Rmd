...
time values. Now we get three columns `fc_point`, `fc_lower`, and `fc_upper`
...
in the call to `epi_slide()`, then we would have gotten a list column `fc`, and
each element of this column would have been a data frame with named columns
...
```{r, message = FALSE, warning = FALSE, fig.width = 9, fig.height = 6}
# Note the use of all_rows = TRUE, so that we get to keep all original rows in
# the returned object
