...
utils::globalVariables(c("is_na", "signal", "issue"))
```
  ...
  if (verbose) {
    ...
    for (col in names(na_regions_list)) {
      ...
      if (nrow(na_regions) > 0) {
        for (i in seq_len(nrow(na_regions))) {
...
for (i in seq_along(cov_nas_subset_epi_ma$confirmed_cov)) {
...
cov_nas_subset_linear_int$confirmed_cov_interpolated <- linear_interpolate_two_pass(cov_nas_subset$confirmed_cov)
