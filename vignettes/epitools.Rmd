...
using `as.epi_signal()`, provided that it has the columns described above. To 
learn more about the `epi_signal` format, you can read the documentation for 
`as.epi_signal()`.
...
* `issue`: the time value at which the given data set was issued (this 
...
`as.epi_signal()`, the others can be omitted. If `issue` is missing, then 
today's date will be used as an issue date. If the `geo_type` or `time_type` 
are missing, then, `as.epi_signal()` will try to guess them from the data.
...
## Stacking data sets into "long" format
