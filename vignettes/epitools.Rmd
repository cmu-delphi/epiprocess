...
As an example, we'll look at daily reported COVID-19 cases for 4 states (CA,
FL, NY, and TX) in the U.S., over a period from mid to late 2020, using the 
...
```{r}
# Convert this into epi_signal format, but this time let as.epi_signal() guess 
# the geo and time types from the data
...
ggplot(x, aes(x = time_value, y = value, color = geo_value)) + 
  ...
  labs(x = "Date", y = "Reported COVID-19 cases", color = "State")
```
