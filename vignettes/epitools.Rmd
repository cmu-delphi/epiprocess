...
We'll start by showing how to get data into `epi_tibble` format, which is just a
tibble with a bit of special structure, and is the format assumed by all of the
functions in the `epitools` package. An `epi_tibble` object has (at least) the
following columns:   
...
refer to as signal variables. To learn more about the `epi_tibble` format, you 
can read the documentation for `as.epi_tibble()`.

A data frame or tibble that has `geo_value` and `time_value` columns can be 
converted into an `epi_tibble` object, using the function `as.epi_tibble()`. As 
...
columns required for an `epi_tibble` object (along with many others). A call to
`as.epi_tibble()`, with further specification of some relevant metadata, brings 
the data frame into `epi_tibble` format.
...
call to `as.epi_tibble()`, then this function will try to infer them from the 
passed object (The documentation for `as.epi_tibble()` gives more details.) 
...
Data in `epi_tibble` format should be easy to work with downstream; in the other
