...
if (!file.exists(ga_hhs_issue_data_path)) {
  ga_hhs_issue_data =
    covidcast(
      "ga_hhs", "confirmed_admissions_influenza_1d",
  ...
  writeLines(c("License: Public Domain US Government","Accessed via COVIDcast Epidata API"),
             "LICENSE_ga_hhs_issue_data.txt")
} else {
  ga_hhs_issue_data = readRDS(ga_hhs_issue_data_path)
}
# TODO use, e.g., cachem.  And explain why caching.
```
...
full_join(version_aware_covariates, forecast_date_edf) %>%
