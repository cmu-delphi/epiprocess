...
p1 <- ggplot(x, aes(x = time_value, y = value)) + 
  geom_line() + 
  geom_point(data = x %>% filter(deriv >= threshold),
             aes(x = time_value, y = value), color = "red") + 
  scale_x_date(minor_breaks = "month", date_labels = "%b %y") +
  labs(x = "Date", y = "Reported COVID-19 cases per 100,00 people") 
...
p1 <- ggplot(x, aes(x = time_value, y = value)) +
  geom_line() + 
  geom_point(data = x %>% filter(deriv_ss1 >= threshold),
             aes(x = time_value, y = value), color = "red") + 
  geom_point(data = x %>% filter(deriv_ss2 >= threshold),
...
result in erratic derivative estimates.
...
p1 <- ggplot(x, aes(x = time_value, y = value)) +
  geom_line() + 
  geom_point(data = x %>% filter(deriv_tf1 >= threshold),
             aes(x = time_value, y = value), color = "red") + 
  geom_point(data = x %>% filter(deriv_tf2 >= threshold),
             aes(x = time_value, y = value), color = "blue", shape = 21) +
  scale_x_date(minor_breaks = "month", date_labels = "%b %y") +
  labs(x = "Date", y = "Reported COVID-19 cases per 100,00 people") 
