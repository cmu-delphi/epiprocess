...
test_that("new setup args and warnings are as expected", {
  ...
  expect_snapshot(
    error = TRUE, 
    growth_rate(y = c(1:20), method = "smooth_spline", 
  ...
  expect_length(growth_rate(
    z, method = "smooth_spline", params = growth_rate_global_params(spar = .5)
  ), 30L)
  expect_length(growth_rate(
    z, method = "smooth_spline", params = growth_rate_global_params(lambda = 10)
  ), 30L)
  expect_length(growth_rate(
    z, method = "smooth_spline", params = growth_rate_global_params(df = 14)
  ), 30L)
  expect_length(growth_rate(
    z, method = "smooth_spline", params = growth_rate_global_params(cv = TRUE)
  ), 30L)
})
